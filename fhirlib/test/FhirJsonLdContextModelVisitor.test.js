...
const {ResourceLoader} = require('../FhirRdfModelGenerator');
...
Jsonld.documentLoader = function(url) {
  if (url.startsWith(FhirJsonLdContextModelVisitor.STEM) && url.endsWith(FhirJsonLdContextModelVisitor.SUFFIX)) {
    ...
    const resourceLoader = new ResourceLoader(R5Resources, R5Datatypes, undefined /* not needed for JSON-LD context generation */);
    const generator = new FhirJsonLdContextModelVisitor(resourceLoader);
    const resourceDef = resourceLoader.getStructureDefinitionByName(genMe, GEN_JSONLD_CONTEXT_CONFIG);
